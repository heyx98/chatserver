...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        case 1:     //login业务
        {
            ...
            js["id"] = id;
            js["password"] = pwd;
            string request = js.dump(); //转成字符串发送  
            ...
            else
            {
                ...
                else 
                {
                    ...
                    else    //登录成功
                    {
                        ...
                        {
                            json grpjs = json::parse(gstr);
                            Group group;
                            group.setId(grpjs["id"].get<int>());
                            group.setName(grpjs["groupname"]);
                            group.setDesc(grpjs["groupdesc"]);

                            vector<string> guvec = grpjs["users"];
                            for(string &gustr : guvec)
                            {
                                GroupUser guser;
                    ...
                    if(responsejs.contains("offlinemsg"))
                    {
                        ...
                        for(string &str : vec)
                        {
                            ...
                            {
                                cout << " 群聊天 [ " << js["groupid"] << " ] " <<js["time"].get<string>() 
...
void showCurrentUserData()
{
    ...
    if(!g_currentUserGroupList.empty())
    {
        for(Group &group : g_currentUserGroupList)
        {
            ...
            {
                cout << guser.getId() << "  " << guser.getName() << "  " << guser.getState() << "  "
                << guser.getRole() << endl;
...
void mainMenu(int clientfd)
{
    ...
    while(isMainMenuRunning)
    {
        ...
        //调用相应的命令的事件处理回调 mainMenu对修改封闭，添加或修改功能不需要修改该函数
...
void addfriend(int clientfd, string str)
{
    ...
    string buffer = js.dump();//将json数据系列化 转化为string 便于发送
